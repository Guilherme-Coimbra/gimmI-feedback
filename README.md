...
5. **Dashboard** (`dashboard.html`)
   ...
   - Preference stacks read like a leaderboard, 1st preference on top. Counts next to each question are **rankings, not votes** — one reviewer comparing three answers casts three pairwise votes, so a raw vote count read 24 where only 8 people had given an opinion.
   - Gradings Overview is one small chart per criterion rather than one grouped chart. Score axes run 1 → 3 → 5 left to right, and every bar is labelled with the criterion's own word for that score ("1 · Hallucinated" under Scientific Accuracy, "1 · Superficial" under Depth of Specialization), taken from `grade_options`. A shared legend could not carry a name that changes per criterion.
   - The two ordinal scales use **different hues** — blue for preference position, amber for scores — so that "pale means best" learned on one chart is never carried across to the other, where the pale end of a shared ramp would have suggested a score of 1 was good.
